...
\author{Malgorzata Nowicka\footnote{gosia.nowicka@uzh.ch}, Mark D. Robinson}
...
\maketitle
\noindent This vignette describes \Rpackage{DRIMSeq} version \Sexpr{packageDescription("DRIMSeq")$Version}.
\tableofcontents
...
Here, we are interesed in the differential analysis between the control and knock-down condition. This information is stored in \Rcode{pasilla\_metadata\$condition}. 

The data frame with counts called \Robject{pasilla\_counts} is already formated in the right way. It contains variables \Rcode{feature\_id} with unique transcript names and \Rcode{gene\_id} with unique gene IDs and columns with counts have the same names as \Rcode{sample\_id} in \Rcode{pasilla\_samples}.
...
When \Rcode{one\_way = TRUE}, multiple group fitting is performed. When \Rcode{one\_way = FALSE}, a regression framework is used with the design matrix defined by a formula ‘~ group’ where group is a continous (not categorical) varialbe with genotype values 0, 1, and 2.
...
<<SQTLplotFit, out.width = "16cm", fig.width = 16>>=
res <- results(d)
res <- res[order(res$pvalue, decreasing = FALSE), ]
