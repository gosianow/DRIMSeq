%\VignetteIndexEntry{Differential transcript usage and transcript usage QTL analyses in RNA-seq with 'DRIMSeq' package}
...
\bioctitle[Differential transcript usage and transcript usage QTL analyses in RNA-seq with \Rpackage{DRIMSeq} package]{DRIMSeq: Dirichlet-multinomial framework for differential transcript usage and transcript usage QTL analyses in RNA-seq}
\author{
Malgorzata Nowicka\thanks{\email{gosia.nowicka@uzh.ch}},
Mark D. Robinson\thanks{\email{second@author.com}}\\
...
\itemize{
\item Implementation of the regression framework in differential transcript usage analysis. It allows fitting more complicated than simple group comparison experimental designs, for example, one can account for the batch effects or the fact that samples are paired or model continous time course changes. It enables also testing of more complicated contrasts.

\item Transcript-level analysis based on the beta-binomial model. In this case, each transcript ratio is modelled separatelly assumming it follows the beta-binomial distribution which is a one-dimensional version of the Dirichlet-multinomial distribution. Based on the fact that when $(Y_1,\ldots,Y_q) ~ DM(\pi_1,\ldots,\pi_q, \gamma_0)$ then $Y_j ~ BB(\pi_j,\gamma_0)$, we do not need to reestimate the beta-binomial parameters, only the likelihoods are recalculated.
\Rpackage{DRIMSeq} returns gene-level and transcript-level p-values which can be used as input to the stage-wise testing procedure \cite{VandenBerge2017} as screening and confirmation p-values, respectively. Such approach provides increased power to identify transcripts which are acctually differentially used in a gene detected as gene with DTU. 

\item Usage of term 'precision' instead of 'dispersion'. In the differential analysis based on the negative-binomial model, dispersion parameter is estimated. This dispersion parameter captures all sources of the inter-library variation between replicates present in the RNA-seq data. 
In the DM model, we do not directly estimate dispersion but a parameter called precision which is closely linked to dispersion via the formula: dispersion = 1 / (1 + precision). In the previous version of \cite{DRIMSeq}, we used 'dispersion' as a name for the functions and variables calculating and storing, in fact, the precision estimates. Now, we use the term 'precision'.
}

\section{Overview of the Dirichlet-multinomial model}

For the statistical details about the Dirichlet-multinomial model, see the \Rpackage{DRIMSeq} paper \cite{DRIMSeq}.
...
\item First, we use the profile likelihood to estimate the precision which is inverse proportional to dispersion, i.e., the variability of transcript ratios between samples (replicates) within conditions. 
...
\item Finally, we fit the null model DM and BB regression and use the likelihood ratio statistics to test for the differences in transcript proportions between the full and null models at the gene and transcript level.
...
\itemize{
\item Currently, transcript-level analysis based on the BB model are implemented only in the DTU analysis. 

\item When the model (full or null) of interest corresponds to multiple (or one) group fitting, then a shortcut algorithm called 'one\_way', which we adapted from the \Rfunction{glmFit} function in \Biocpkg{edgeR}, can be used. Choosing it is equivalent to running the original \Rpackage{DRIMSeq} implementation. In such a case, we use maximum likelihood to estimate the transcript proportions in each group separately and then the regression coefficients are calculated using matrix operations. Otherwise, the regression coefficints are directly estimated with the maximum likelihood approach.
}
...
Here is a list of functions and parameters for which no default values are predefined, and in order to proceed with the pipeline, the user has to provide values for these parameters:

\itemize{
\item \Rfunction{dmDSdata}: \Robject{counts}, \Robject{samples},
\item \Rfunction{dmSQTLdata}: \Robject{counts}, \Robject{gene_ranges}, \Robject{genotypes}, \Robject{snp_ranges}, \Robject{samples},
\item \Rfunction{dmPrecision}: \Robject{x}, \Robject{design},
\item \Rfunction{dmFit}: \Robject{x}, \Robject{design},
\item \Rfunction{dmTest}: \Robject{x}, \Robject{coef} or \Robject{design} or \Robject{contrast}.
}
...
\section{DRIMSeq differential transcript usage analysis workflow}
...
The RNA-seq data can be downloaded from the NCBI’s Gene Expression Omnibus (GEO) under the accession 
number GSE18508. 
In the examples below, we use a subset of \software{kallisto} \cite{Bray} counts available in \Biocexptpkg{PasillaTranscriptExpr} package, where you can find all the steps needed, for preprocessing the GEO data, to get a table with transcript counts.
...
To create a \Rcode{dmDSdata} object, saved as variable \Robject{d}, we need to prepare a data frame containing information about samples and we will call it \Robject{pasilla_samples}. 
It has to have a variable called \Rcode{sample_id} with unique sample names that are identical to column names in \Robject{pasilla_counts} that correspond to samples. 
Additionally, it has to contain other variables that the user would like to use for the further regression analysis. 
Here, we are interesed in the differential analysis between the control and knock-down condition. This information is stored in \Rcode{pasilla_metadata$condition}. 

The data frame with counts called \Robject{pasilla_counts} is already formated in the right way. It contains variables \Rcode{feature_id} with unique transcript names and \Rcode{gene_id} with unique gene IDs and columns with counts have the same names as \Rcode{sample_id} in \Rcode{pasilla_samples}.
...
Since we have 3 knock-down and 4 control samples, we set \Robject{min\_samps\_feature\_expr} equal to 3. In this way, we allowa situation where a transcript is expressed in one condition but not in another, which is a case of differential transcript usage. 
...
In the default setting, all of them are computed and common precision is used as an initial value in the grid approach to estimate gene-wise precisions, which are shrunk toward the trended precision.

By default, to estimate the common precision, we use 10\% percent (\Rcode{prec_subset = 0.1}) of randomly selected genes. That is due to the fact that common precision is used only as an initial value, and estimating it based on all the genes takes a substantial part of time. 
...
To estimate precision parameters, the user has to define a design matrix with the full model of interest, which will be also used later in the proportion estimation. Here, the full model is defined by a formula \Rcode{~ group} which indicates that samples come from two conditions.
...
In RNA-seq data, we can typically observe a trend where the dispersion decreases (here, presicion increases) for genes with higher mean expression.
...
By default, \Rcode{one_way = TRUE} which means that whenever the design corresponds to multiple group fitting, the 'one way' shortcut algorith will be used, which in fact corresponds to the first implementation of \Rpackage{DRIMSeq}. Transcript proportions are estimated for each condition separately and then the regression coefficients are calculated using matrix operations. 
By setting \Rcode{verbose = 1}, we can see that the one way approach is used with the current desing.

When \Rcode{bb_model = TRUE} (the default), additionally to the gene-level Dirichlet-multinomial estimates the transcript-level beta-binomial results will be computed.  
...
Both steps are done at the gene and transcript level when \Rcode{bb_model = TRUE}.
...
We can achieve that by using the \Rcode{coef} parameter which should indicate which columns of the full design should be removed to get the null design. We define it equal to \Rcode{"groupKD"}. Then the null design has only an interecpt column which means that all the samples are treated as if they came from one condition. 
Note that \Rcode{one_way = TRUE} and the one way approach is used.
...
Or by using the \Rcode{contrast} parameter.
...
You have to define the \Rcode{group_variable} parameter which should indicate a variable from \Rcode{samples(d)}. 
...
The regression framework implemented in \Rpackage{DRIMSeq} allows to account for the batch effects. Here, this would be the library layout stored in \Rcode{pasilla_metadata$LibraryLayout}. 
...
<<DRIMSeq_batch, eval = FALSE, out.width = "16cm", fig.width = 16>>=
...
set.seed(123)
## Calculate precision
d2 <- dmPrecision(d2, design = design_full2, prec_interval = c(0, 1e+02))
plotPrecision(d2)
## Fit proportions
...
%------------------------------------------------------------------------------
% sQTL analysis workflow
%------------------------------------------------------------------------------


\section{sQTL analysis workflow}
...
When ‘one_way = TRUE’, multiple group fitting is performed. When ‘one_way = FALSE’, a regression framework is used with the design matrix defined by a formula ‘~ group’ where group is a continous (not categorical) varialbe with genotype values 0, 1, and 2.
...
If \Rcode{speed} is set to \Rcode{TRUE}, there will be only one precision calculated per gene (assumming a null model, i.e., model with intercept only), and it will be assigned to all the blocks matched to this gene.
...
In contrast to the DTU analysis, there are some additional challenges that have to handeled in the tuQTL analysis. They include a large number of tests per gene with highly variable allele frequencies (models) and linkage disequilibrium. As in other sQTL studies, we apply a permutation approach to empirically assess the null distribution of associations and use it for the adjustment of nominal p-values.

There are two permutation schemes available. 
When \Rcode{permutation_mode} equals to \Rcode{"all_genes"}, the null p-value distribution is calculated from all the genes.
When \Rcode{permutation_mode = "per_gene"}, null distribution of p-values is calculated for each gene separately based on permutations of this individual gene. 
