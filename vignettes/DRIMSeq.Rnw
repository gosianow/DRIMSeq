...
When \Rcode{one\_way = TRUE}, multiple group fitting is performed. When \Rcode{one\_way = FALSE}, a regression framework is used with the design matrix defined by a formula \Rcode{\~ group} where \Rcode{group} is a continuous (not categorical) variable with genotype values 0, 1, and 2.
